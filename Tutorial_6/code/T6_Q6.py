...
# onehot_encoder=OneHotEncoder(sparse=False) # Use sparse_output instead of sparse
...
X_train_lin = np.concatenate((bias1, X_train), axis = 1) 

Bias2 = np.ones((X_test.shape[0], 1)) 
X_test_lin = np.concatenate((Bias2, X_test), axis = 1) 
w = inv(X_train_lin.T @ X_train_lin) @ X_train_lin.T @ Ytr_onehot 
print(w) 
...
print(len(correct)) 
print(f'accuracy of LC={accuracy}\n')
